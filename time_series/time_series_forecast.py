...
class TimeSeriesDataFrameParser:
    ...
    def parse_one_column(self, idx: int, copy=True) -> pd.DataFrame:
        values = [
            pd.to_datetime(self.df.index).values,
            np.ones(self.df.shape[0], dtype=int) * idx,
            self.df.iloc[:, idx].values.astype(float),
        ]
    ...
    def parse(self) -> pd.DataFrame:
        if self.parsed is None:
            self.parsed = pd.concat(
                [self.parse_one_column(i) for i in range(len(self.df.columns))]
            )
        return self.parsed
...
def get_default_stats_vol_models():
    models = [stats_models.GARCH(1, 1), stats_models.ARCH()]
    return models
